...
static int			ft_btree_add_below(t_btree *btree, t_named *item)
{
	int				cmpr;

	cmpr = btree->cmp(item->key, btree->named->key);
	if (cmpr < 0)
	{
		if (btree->left == NULL)
		{
	...
	char *key, t_btree **parent)
{
	...
	*parent = NULL;
	cur = btree;
	while ((cmpr = cur->cmp(key, cur->named->key)))
	{
		*parent = cur;
		if (cmpr < 0 && cur->left)
			cur = cur->left;
...
t_named				*ft_btree_get_named(t_btree *btree, char *key)
{
	...
	return ((target == NULL) ? NULL : target->named);
}
...
static void			ft_btree_remove_left_sided(t_btree *parent, t_btree *target)
{
	...
	if (parent == NULL)
	{
		target->named = target->left->named;
	...
	t_btree *target)
{
	...
	if (parent == NULL)
	{
		target->named = target->right->named;
...
int					ft_btree_fill_copy(t_btree *old_btree, t_btree *new_btree)
{
	...
	out = ft_btree_add(new_btree, old_btree->named);
...
int					ft_btree_fill_array(t_btree *btree, t_array *array)
{
	...
	out = ft_array_add(array, btree->named);
